...
for i in range(1,nmax-1):
    ...
    if(i > nmax/2):
        wave.t1[i,1] = nmax/2*Ai*dx- Ai*dx*(i-nmax/2)
...
def wave_stiff_damp(nmax,wave):
    ...
    ymi2 = -wave.t1[1,1] # Current y at 1-2, which there is non at this bound so we take the inverse of the current
    ...
    for i in range(2,nmax-2):
        ...
        #wave.pos[i,1]= ((2*y)/t**2+c**2 *((-2*y+yi1+ymi1)/x**2-(en*(6*y*-4*yi1+yi2- 4*ymi1+ymi2))/x**4)-ymn1/t**2+b*ymn1/t)/(1/t**2+b/t)
    ...
    yi2 = -wave.t1[n,1] # Current y at n+2, which there is non at this bound so we take the inverse of the current
